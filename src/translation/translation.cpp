...
namespace util {
    ...
    void translation::build_translation_map() {
        std::filesystem::path path{ fmt::format(R"(Data\Interface\Translations\SkyrimProjectGlossary_{}.txt)",
            get_game_language()) };

        if (!load_translation(path)) {
            load_translation(R"(Data\Interface\Translations\SkyrimProjectGlossary_ENGLISH.txt)"sv);
        }
    }
